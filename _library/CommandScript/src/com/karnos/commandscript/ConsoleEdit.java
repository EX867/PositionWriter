...
public class ConsoleEdit extends TextEdit {
    ...
    content = new EditorString() {
      ...
      public void delete(int i, int i1, int i2, int i3) {//do not allow user to erase things!
        ...
        if (line < editingLine) {
          line = editingLine;
          point = 0;
        } else if (point > getLine(editingLine).length()) {
          point = getLine(editingLine).length();
        }
      }
      @Override
  ...
  public void keyTyped(KeyEvent e) {
    if (KyUI.focus == this) {
      ...
      if (e.getKey() != PApplet.CODED && !KyUI.ctrlPressed && !KyUI.altPressed) {
        if (content.line < editingLine) {
          content.line = editingLine;
          content.point = Math.min(header.length(), getLine(editingLine).length());
        } else if (content.point > getLine(editingLine).length()) {
          content.point = getLine(editingLine).length();
        }
      }
      if (e.getKey() == '\n') {
        if (processor != null) {
