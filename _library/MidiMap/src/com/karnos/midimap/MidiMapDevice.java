...
public class MidiMapDevice implements Receiver {//this module ignores channel.
  ...
  public static MidiMapDevice enableOutput(String name_, String mapFile, javax.sound.midi.Receiver outputReceiver_, int index_) {
    MidiMapDevice d = createDevice(name_, index_);
    d.enableOutput(name_, mapFile, outputReceiver_);
    d.index = index_;
    d.output = true;
    return d;
  }
  protected void enableOutput(String device, String mapFile, javax.sound.midi.Receiver outputReceiver_) {
    outputReceiver = outputReceiver_;//enable output
    loadOutputMap(device, mapFile, index);
    setState(state);
  }
  protected void loadInputMap(String path, int index) throws RuntimeException {//get file path, must be xml.
    if (applet == null) {
      System.out.println("MidiMapDevice : failed");
      return;
    }
    //<index_0> - index
    ...
    if (getFileExtension(path).equals("xml")) {
      ...
      for (XML xml : states) {
        if (!midiMaps.containsKey(xml.getString("value"))) {
  ...
  protected void loadOutputMap(String device, String path, int index) throws RuntimeException {//get file path, must be xml.
      ...
      for (XML xml : states) {
        ...
        for (XML command : commands) {
          ...
          try {
            ...
            MidiCommand.addOutput(/*add state to name*/stateValue + ":" + device + ":" + command.getContent(), new OutputBehavior(outputReceiver, message));//direct add.is note_on and note_off.
            //}
