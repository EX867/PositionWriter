  ...
  if (file.isDirectory()) {//and exists
    try {
      ...
      if (infof.isFile()) {
        ...
        UnipackInfo info=loadUnipackInfo(filename, readFile(infof.getAbsolutePath()));
      ...
      if (keySoundf.isFile()) {//soundsf.isDirectory()
        ...
        KsProcesser proc=new KsProcesser();
        Script ksScript=new Script(getFileName(keySoundf.getAbsolutePath()), ksCommands, proc);
        ksScript.insert(0, 0, readFile(keySoundf.getAbsolutePath()));
        ksScript.readAll();
        ArrayList<Command> lines=ksScript.getCommands();
        //int a=0;
        //while (a<lines.size()) {
      ...
      if (keyLEDf.isDirectory()) {
        ...
        //ArrayList<UnipackLine> lines=analyzer.loadKeySound(keyLEDf.getAbsolutePath());
        //int a=0;
        //while (a<lines.size()) {
        //  UnipackLine line=lines.get(a);
        //  if (0<line.x&&line.x<=ButtonX&&0<line.y&&line.y<=ButtonY&&0<line.chain&&line.chain<=Chain) {
        //    if (line.absolute) {
