...
class LedTab {
  ...
  LedTab(LedScript script) {
    ...
    if (Device.getDeviceByName("Launchpad MK2")!=null) {
      light.device=Device.getDeviceByName("Launchpad MK2");//TEST
      light.device.setState(script.info.buttonY+"x"+script.info.buttonX);
